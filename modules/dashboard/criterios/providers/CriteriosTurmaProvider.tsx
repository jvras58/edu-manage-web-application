...
import {
  Criterio,
  TurmaCriterios,
} from '@/modules/dashboard/criterios/types/criterios.types';
...
const CriteriosTurmaContext = createContext<
  CriteriosTurmaContextType | undefined
>(undefined);
...
export function CriteriosTurmaProvider({
  ...
  const [deletingCriterio, setDeletingCriterio] = useState<Criterio | null>(
    null
  );
...
export function useCriteriosTurma() {
  ...
  if (context === undefined) {
    throw new Error(
      'useCriteriosTurma must be used within a CriteriosTurmaProvider'
    );
  }
  return context;
}
